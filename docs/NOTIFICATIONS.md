...
- **As of v0.8.2, automatic delivery is not enabled in production**: the worker CLI requires the `tsx` runtime, which is not installed on the production host, so no worker is scheduled. Expiration-reminder evaluation + delivery are code-complete and tested; they activate once the runtime dependency is installed and `pnpm worker` is scheduled (e.g. cron).
