...
> Verified recovery procedures and honest status of each scenario (updated 2026-08-19 for v0.8.3).
...
1. Reinstall runtime: Node ≥22, pnpm (corepack), project checkout at the v0.8.3 release commit, `pnpm install --frozen-lockfile`, `pnpm build`
...
- Code source of truth: GitHub `hxx0611/Domain-Monitor`, main = v0.8.3 release commit (see `git rev-parse origin/main`). Re-clone, `pnpm install --frozen-lockfile`, `pnpm build`. No local-only code exists.
...
## v0.8.3 deployment order (release/deploy gate)

1. Apply pending migrations — none for v0.8.3 (schema unchanged since 0007, which is registered in the journal since Phase 11E; a fresh environment runs `pnpm db:migrate` end-to-end 0000→0007)
